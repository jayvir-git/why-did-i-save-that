...
RULES += [
 ...
 ('D66','Boundaries','Providers receive selected input','Codex uses an ephemeral read-only sandbox without user configuration. Claude uses a temporary directory, safe mode, no built-in tools, empty strict MCP and no session persistence. Selected text and a supported cached image are supplied.','The selected source is sent to the signed-in provider and consumes account usage. The UI explains this at the Start action.','Inspect the spawned argument vector and prompt; no whole-library prompt or workspace-write permission.','gold_workspace/runner.py',"args=[executable,'exec'"),
]

RULES += [
 ('D67','Setup','Baseline setup is repeatable','Default setup checks Python and SQLite, runs a disposable workflow and generates absolute-path MCP configuration without downloads or provider calls.','Optional packages/assets require explicit --with flags. Sign-in and collection remain human steps.','Run setup twice in a clean copy from another directory.','scripts/setup.py','def setup'),
 ('D68','Reviews','Provider choice persists with the run','Review provider is codex or claude. Old runs default to Codex; one active attempt per review deduplicates across providers.','Retry retains the provider. Switching an active review requires cancellation.','Start Claude, request Codex on the same review and observe the existing Claude run.',WORKFLOWS,'def start_work'),
 ('D69','Setup','Fresh acceptance uses no personal archive','A clean temporary checkout omits runtime, models and private data, runs setup twice and launches the generated MCP command from elsewhere.','Offline baseline tests do not prove optional installs or live provider authentication.','Run check_fresh_install.py; no models are downloaded.','scripts/check_fresh_install.py','def check'),
 ('D70','Setup','Agent instructions have explicit entry points','AGENTS.md and CLAUDE.md link to one setup guide with commands, capabilities, human handoffs and verification limits.','A remote-only harness cannot access a local archive without a supported connection.','Follow the baseline prompt using shell or local stdio MCP.','docs/agent-start.md','# Start with your agent'),
]
...
TESTS.update({
 'D67':('tests/test_setup.py','def test_mcp_config'), 'D68':('tests/test_providers.py','def test_provider_persists'), 'D69':('scripts/check_fresh_install.py','def check'),
...
DIAGRAMS.append({'id':'setup','title':'Agent setup and first use','question':'Can a fresh agent use this workspace without guessing?',
 'nodes':[('entry','Read agent entry point',['D70'],'Agent','Shared setup guide'),('baseline','Run setup',['D67'],'Local Python','Prerequisite checks'),('demo','Verify disposable workflow',['D69'],'CLI + MCP','Synthetic search and cited note'),('config','Generate connection',['D67'],'Setup','Absolute-path MCP config'),('human','Connect personal capabilities',['D70'],'Person','Sign-in, pairing, collection choice'),('use','Use workspace',['D68','D64'],'Agent + person','Search, notes, optional tracked review')],
 'edges':[('entry','baseline','One setup command'),('baseline','demo','No provider calls'),('demo','config','Only after verification'),('config','human','Explicit handoffs'),('human','use','Authorized data and provider')],
...
reviews['nodes']=[('choose','Choose exact source',['D38','D63'],'Person','Selected observation ID'),('pending','Queue pending review',['D39'],'Server','Media request + receipt'),('starting','Choose provider and start',['D40','D66','D68'],'Person + local worker','Run ID'),('running','Provider acknowledged',['D62'],'Provider process','Heartbeat + last activity'),('blocked','Blocked / failed',['D45','D62'],'Worker','Persistent error and retry path'),('draft','Draft ready',['D40','D62'],'Provider + worker','Unpublished draft'),('complete','Inspect and save review',['D41','D42','D64'],'Person + server','Atomic evidence + completion receipt'),('stopped','Cancelled / interrupted',['D62'],'Person / heartbeat monitor','Terminal attempt; late result ignored')]
reviews['edges']=[('choose','pending','Queue selected source'),('pending','starting','Start selected provider'),('starting','running','Codex thread/turn or Claude init'),('starting','blocked','Setup failure'),('running','blocked','Observed provider failure'),('running','draft','Valid structured result + completion'),('draft','complete','User inspects and saves'),('pending','complete','Manual review'),('running','stopped','Cancel or heartbeat expiry')]
...
DELIVERED = {
 'G01':('Local Codex/Claude runners with persistent acknowledgment, heartbeat, quota/configuration errors, cancellation and drafts.','Selected CLI must be installed and signed in. Live Claude verification remains pending. External sessions remain unobserved.'),
...
def build_data():
    ...
    scenarios[0].update(steps=['Queue the selected source','Choose provider and start explicitly','Provider returns a usage error','Worker persists blocked status and retry guidance'],conclusion='Runs started here are observable; external agent sessions remain outside this app.')
